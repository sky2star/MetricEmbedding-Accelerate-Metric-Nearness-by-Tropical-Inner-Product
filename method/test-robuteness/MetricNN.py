...
class MetricNN(nn.Module):
    def __init__(self, N, mean):
        ...
        # 定义多个TAlayer
        init_scale = mean / d /2
        self.x = nn.Parameter(torch.rand(N, 5)*init_scale)
#        self.layer1 = TAlayer(N, 256, init_scale)
        self.layer1 = TAlayer(5, 10, init_scale)
        self.b1 = nn.Parameter(torch.randn(10) * init_scale)
        self.layer2 = TAlayer(10, k, init_scale)
        
        # 定义第二个矩阵B
        #self.B = nn.Parameter(torch.randn(N, k) * init_scale)
...
#        out3 = self.layer3(out2)
        ...
        dist_AB = self.tropical_dot(out2, out2)
        return dist_AB
